    ...
    private fun post() {
        this.lifecycleScope.launch {
            ...
            if (title.isEmpty() || title.isBlank()) {
                Toast.makeText(this@PostNewsActivity, "Укажите заголовок!", Toast.LENGTH_SHORT).show()
                return@launch
            }
            val text = binding.newsText.text.toString().trim()
            if (text.isEmpty() || text.isBlank()) {
                Toast.makeText(this@PostNewsActivity, "Укажите текст новости!", Toast.LENGTH_SHORT).show()
                return@launch
            }
